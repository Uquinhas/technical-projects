...
<html>
	<head>
        <meta charset="UTF-8" />
        <title>Nobre Financeira - Cadastro de Pagamentos </title>
	...
	<body>
        ...
        <div id="botao-container">
			<a href="tabela_pagamento.php">Ver Pagamentos</a>
		</div>
			<h2>Cadastro de Pagamentos</h2>
			<div id="form-div">  

				<!--**********************************Começa o Formulário***********************************************-->
				<form method="post" action="action_pagamento.php" id="fContato" > 
					<fieldset id="formadepagamento">              
						...
						<select name="tFormpag" style="border: solid #999999 1px;">
							...
							<?php 
								$sql = "SELECT * FROM `tb_tipopagamento`";
								$opcao = mysqli_query($con, $sql);
								
								if(mysqli_num_rows($opcao) > 0){	
									while($dados = mysqli_fetch_assoc($opcao)){
										echo '<option value="'.utf8_encode($dados["tipoPagamento"]).'">'.utf8_encode($dados["tipoPagamento"]).'</option>';
									}
								}							
							?>
						</select>
					...
					<fieldset id="DiadoPagamento">   <br />         
						<p> 
							<h1>Dia do Pagamento</h1> <br />
							<input type="date" name="tDia" id="cDia" required/> 
						</p>
					</fieldset><br />
					<fieldset id="valoraPagar">              
						<p>
							<label for="cTot"> <h1>Valor do Pagamento</h1> <br /> R$ </label>
